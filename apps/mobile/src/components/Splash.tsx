...
export function Splash({ visible, onDone }: Props) {
  ...
  useEffect(() => {
    ...
    const sequence = Animated.sequence([
      Animated.parallel([reveal(first, 220), reveal(second, 640)]),
      Animated.delay(900),
      Animated.timing(out, {
        toValue: 1,
        duration: 520,
  ...
  }, [visible, shown, first, second, out, onDone]);
  ...
  return (
    ...
    >
      ...
      <Animated.View
        style={[styles.fill, { opacity: range(1, 0) }]}
        pointerEvents="none"
      >
        <View style={styles.centre}>
          ...
          <View style={styles.lines}>
            ...
            <Animated.Text style={[styles.line, lineStyle(second)]}>
              Forget the rest.
            </Animated.Text>
          </View>
        </View>
...
const makeStyles = (theme: ColorScheme) =>
  StyleSheet.create({
    ...
    lines: {
      ...
      left: 0,
      right: 0,
      paddingTop: MARK_HEIGHT / 2 + space['2xl'],
      paddingHorizontal: space.xl,
      alignItems: 'center',
    },
    line: {
